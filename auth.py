...
def users_stats():
    ...
    joker_rows = conn.execute('SELECT prenom, count FROM user_jokers WHERE subject=?',
                              (subject,)).fetchall()
    ...
    # Scoped on the admin's subject so multi-matieres stocks don't mix.
    streak_by_prenom = {p: compute_streak(conn, p, subject) for p in users}
